...
function Profile() {
  ...
  const subtitle = [
    profile?.programme,
    profile?.level ? `L${profile.level}` : null,
    profile?.university,
  ]
    .filter(Boolean)
    .join(" · ");
  ...
  const handleRefer = () => {
    const origin =
      typeof window !== "undefined" ? window.location.origin : "https://hostelease.app";
        ...
        <div className="overflow-hidden rounded-2xl bg-card shadow-card">
          {BOOKINGS.map((b, i) => (
            ...
            >
              ...
              <span className={`rounded-full px-2.5 py-1 text-[11px] font-semibold ${b.color}`}>
                {b.status}
              </span>
            </div>
          ))}
        ...
        {referralLink && (
          <div className="rounded-2xl border border-primary/30 bg-primary/5 p-4">
            ...
            <p className="mt-1 text-xs text-muted-foreground">
              Share this link. You earn <strong>GHS 20</strong> when your friend completes their
              first booking.
            </p>
...
function Row({
  ...
  const content = (
    <div className="flex items-center gap-3 px-4 py-3.5 [&+&]:border-t [&+&]:border-border">
      <span
        className={`grid h-9 w-9 place-items-center rounded-full ${danger ? "bg-destructive/10 text-destructive" : "bg-accent text-primary"}`}
      >
  ...
  if (to)
    return (
      <Link to={to} className="block border-t border-border first:border-t-0">
        {content}
      </Link>
    );
  if (onClick)
    return (
      <button
        type="button"
        onClick={onClick}
        className="block w-full text-left border-t border-border first:border-t-0"
      >
        {content}
      </button>
    );
